...
let domString = '';
    for(let i = 0; i < monkeybutt.length; i++) {
        domString += `<div class= "pie-ingredients">`
        domString += `<h4>${monkeybutt[i].name}</h4>`;
        domString += `<p>${monkeybutt[i].bakeTemp}</p>`;
        domString += `<p>${monkeybutt[i].instructor}</p>`;
        domString += `</div>`;
    };
